...
function Body() {
  ...
  return (
    ...
    <div className="w-4/6 h-full my-0 mx-auto shadow-2xl">
      ...
      <div className="flex flex-row overflow-x-auto hover:overflow-scroll">
        {date && date?.map( date =><button className="bg-gray-200 list-none rounded-full m-3 p-3 hover:bg-red-500 focus:bg-red-500 border-b-4" value={date} onClick={(e) => setSelectedDate(e.target.value)} key={date}>{date}</button> )}
      </div>
...
const MatchComponent = () => {
  ...
  return (
    <div className="w-4/6 h-full p-8 bg-slate-400 my-0 mx-auto">
  ...
  <tbody>
   ...
   bet builder odds:{betBuilderData.TotalOdds}
    </div>
  )
}
